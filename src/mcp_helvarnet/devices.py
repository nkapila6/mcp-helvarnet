...
def register_device_tools(mcp: FastMCP, get_router: Callable[[], Router]):
    ...
    def get_device_overview(
        device_address: Annotated[str, Field(description="Device address in format like '1.1.2.3'"
                                             "(block.router.subnet.device)")]
    ) -> Dict[str, Any]:
    ...
    def get_devices_by_health_status(
        status: Annotated[str, Field(description="Health status to filter by: 'healthy' "
                                     "or 'issues'")] = "issues"
    ) -> Dict[str, Any]:
    ...
    def get_devices_by_brightness_range(
        min_brightness: Annotated[int, Field(description="Minimum brightness percentage",
                                             ge=0, le=100)] = 0,
        max_brightness: Annotated[int, Field(description="Maximum brightness percentage",
                                             ge=0, le=100)] = 100
    ) -> Dict[str, Any]:
        ...
        try:
            ...
            return {
                ...
                'summary': (f"Found {len(filtered_devices)} devices with "
                            f"brightness between {min_brightness}% and {max_brightness}%")
            }
    ...
    def get_devices_by_protocol(
        protocol: Annotated[str, Field(description="Protocol to filter by "
                                       "(e.g., 'DALI', 'DIM', etc.)")]
    ) -> Dict[str, Any]:
...
def _get_device_overview_internal(device) -> dict[str]:
    """
...
def _get_all_devices_overview_internal(router):
    ...
    return {
        ...
        'quick_summary': (f"{len(light_devices)} devices, {health_summary['healthy']} healthy, "
                          f"avg {avg_brightness:.1f}% brightness")
    }
